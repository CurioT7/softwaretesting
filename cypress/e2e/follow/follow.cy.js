...
describe('Follow test site', () => {
    ...
    it('follow and unfollow a user', () => {
        ...
        FollowPage.unfollowMessage.should('user unfollowed succesfully')
        FollowPage.followUnfollowButton.first().should('be.visible')
    })

    it('check followers count of a user', () => {
        cy.visit('')
        FollowPage.followers.first().should('be.visible').click()
        cy.wait(3000)
        let oldFollowCount
    ...
    it('get followers list', () => {
        ...
        FollowPage.followersList.then(($el) => {
            if($el.find('> [data-testid="followers"]').length > 0){
